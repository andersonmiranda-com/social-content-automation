...
from tools.google_sheets_tool import save_to_sheet_chain
...
social_post_pipeline = (
    # Step 0: Preprocess input to extract the topic
    RunnableLambda(_preprocess_input)
    # Step 1: Generate post data, add it to the bag as 'post_data'
    | RunnablePassthrough.assign(post_data=generate_post_chain)
    # Step 2: Generate image data from 'post_data', add it as 'image_data'
    | RunnablePassthrough.assign(
        image_data=(RunnableLambda(lambda x: x["post_data"]) | generate_image_chain)
    )
    # Step 3: Upload image and assign just the final URL to a new key
    | RunnablePassthrough.assign(
        final_image_url=(
    ...
    # Step 4: Pass the whole bag to the final saving step.
    | save_to_sheet_chain
)
